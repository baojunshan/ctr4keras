...
from ctr4keras.models import DeepFM
...
def main():
    ...
    data = fillna(data, preprocessor.features)
    df = preprocessor.fit_transform(data)

    print(df.head())

    # 2. build model
    model = DeepFM(features=preprocessor.features, dense_emb_dim=4, dense_hidden_dims=[128, 128, 32], regularizer=5e-4)
    model.summary()
    model.compile(
    ...
    plot = Plot(title="DeepFM eval metrics")
    for t in ["loss", "auc"]:
        plot.add_plot(
            label2dpair={
