...
from ctr4keras.snippets import evaluate_report, binary_focal_loss, HistoryExt, Plot
...
def main():
    ...
    data = pd.read_parquet("../data/douya_train_data.parquet")

    cont_cols = [c for c in data.columns if c.startswith("ftr_cont")]
    cate_cols = [c for c in data.columns if c.startswith("ftr_cate")]
    data[cont_cols] = data[cont_cols].fillna(0)
    data[cate_cols] = data[cate_cols].fillna("null")
    data = data.replace(True, "true")
    data = data.replace(False, "false")

    # 2. feature process (minmax scale and label encoding)
    feat_type_dict = dict()
    for i in data.columns:
        if i == "label":
            continue
        if i.startswith("ftr_cate"):
            feat_type_dict[i] = FeatureType.Categorical
        elif i.startswith("ftr_cont"):
            feat_type_dict[i] = FeatureType.Continuous
        elif i.startswith("ftr_multi_cate"):
